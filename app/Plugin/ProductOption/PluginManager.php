...
class PluginManager extends AbstractPluginManager
{
    ...
    public function enable($config, $app)
    {
        $this->addCsv($app);
        $this->addCSVext($app);
        $this->addCheckBox($app);
        $this->addDateType($app);
        $this->addNumberType($app);
    }
    ...
    public function update($config, $app)
    {
        ...
        if(version_compare($Plugin->getVersion(),'1.6.0','<=')){
            $this->addDateType($app);
            $this->addNumberType($app);
        }

        if(version_compare($Plugin->getVersion(),'1.6.1','<=')){
            $OrderOptionItems = $app['orm.em']->getRepository('Plugin\ProductOption\Entity\OrderOptionItem')->findAll();
            foreach($OrderOptionItems as $OrderOptionItem){
                $Option = $OrderOptionItem->getOption();
                if(!is_null($Option)){
                    $OrderOptionItem->setOptionType($Option->getType()->getId());
                    $app['orm.em']->persist($OrderOptionItem);
                }
            }
            $app['orm.em']->flush();
        }
    ...
    private function addCheckBox($app)
    {
        $CheckboxType = $app['orm.em']->getRepository('Plugin\ProductOption\Entity\Master\Type')->find(5);
        if(is_null($CheckboxType)){
            $MasterType = new \Plugin\ProductOption\Entity\Master\Type();
            $MasterType->setId(5)
                       ->setName('チェックボックス')
                       ->setRank(3);
            $app['orm.em']->persist($MasterType);
            $MasterType = $app['orm.em']->getRepository('Plugin\ProductOption\Entity\Master\Type')->find(3);
            $MasterType->setRank(4);
            $app['orm.em']->persist($MasterType);
            $MasterType = $app['orm.em']->getRepository('Plugin\ProductOption\Entity\Master\Type')->find(4);
            $MasterType->setRank(5);
            $app['orm.em']->persist($MasterType);
    ...
    private function addDateType($app)
    {
        $CheckboxType = $app['orm.em']->getRepository('Plugin\ProductOption\Entity\Master\Type')->find(6);
        if(is_null($CheckboxType)){
            $MasterType = new \Plugin\ProductOption\Entity\Master\Type();
            $MasterType->setId(6)
                       ->setName('日付')
    ...
    private function addNumberType($app)
    {
        $NumberType = $app['orm.em']->getRepository('Plugin\ProductOption\Entity\Master\Type')->find(7);
        if(is_null($NumberType)){
            $MasterType = new \Plugin\ProductOption\Entity\Master\Type();
            $MasterType->setId(7)
                       ->setName('数値入力')
                       ->setRank(7);
