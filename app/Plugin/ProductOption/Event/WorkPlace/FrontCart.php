...
class FrontCart extends AbstractWorkPlace
{
    public function createTwig(TemplateEvent $event)
    {
        ...
        $is_delivery_free = $parameters['is_delivery_free'];
        ...
        foreach($cartItemOptions as $cartItemOption){
            if($cartItemOption->getDeliveryFreeFlg()){
                $deliv_free_flg = true;
                break;
            }
        }
        ...
        if($deliv_free_flg){
            ...
            if(!$BaseInfo->getDeliveryFreeAmount() && !$BaseInfo->getDeliveryFreeQuantity()){
                if(preg_match('/BaseInfo\.delivery_free_amount\sand\sBaseInfo\.delivery_free_quantity/',$source, $result)){
                    $search = $result[0];
                    $replace = 'true';
                    $source = str_replace($search, $replace, $source);
                }
        ...
        $parameters['is_delivery_free'] = $is_delivery_free;
