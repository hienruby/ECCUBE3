...
class AdminOrderEditLegacy extends AbstractWorkPlace
{
    public function createTwig(TemplateEvent $event)
    {
                        ...
                        if ($Option) {
                            ...
                            if ($Option->getType()->getId() == 1 || $Option->getType()->getId() == 2) {
                                if ($Option->getDisableCategory()) {
                                    if ($Option->getDisableCategory()->getId() == $option_value) {
                                        $add = false;
                                    }
                                }
                            } elseif($Option->getType()->getId() == 5){
                                if(count($option_value) == 0)$add = false;
                            } elseif ($Option->getType()->getId() == 3 || $Option->getType()->getId() == 4) {
                                if (strlen($option_value) == 0)
                                    $add = false;
                            }
                            if ($add) {
                                $Options['productoption' . $option['option_id']] = $option_value;
                            }
                        }
                    }
                }
        ...
        foreach($formOrderDetails as $formOrderDetail){
            ...
            $plgOrderDetails[$formOrderDetail->getIndex()] = $app['eccube.productoption.service.util']->getLabelFromOptions($Options,true);
        }

        foreach($formShipmentItems as $formShipmentItem){
            $Options = unserialize($formShipmentItem->getSerial());
            $plgShipmentItems[$formShipmentItem->getShippingIndex()][$formShipmentItem->getItemIndex()] = $app['eccube.productoption.service.util']->getLabelFromOptions($Options,true);
        }
        ...
        if(preg_match('/<(.*)\s*id="product_info_list__class_category_name.*>/',$source, $result)){
            $start_tag = $result[0];
