...
class OptionType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder
                ...
                ->add('require_min', 'integer', array(
                    'label' => '必須選択数(下限)',
                    'constraints' => array(
                        new Assert\GreaterThanOrEqual(array('value' => 0)),
                    ),
                ))
                ->add('require_max', 'integer', array(
                    'label' => '必須選択数(上限)',
                    'constraints' => array(
                        new Assert\GreaterThanOrEqual(array('value' => 0)),
                    ),
        ...
        $builder->addEventListener(FormEvents::POST_SUBMIT, function (FormEvent $event) {
            ...
            if(strlen($min) > 0 && strlen($max) > 0 && $min > $max){
                $form['require_min']->addError(new FormError('指定範囲を正しく設定してください。'));
                $form['require_max']->addError(new FormError('指定範囲を正しく設定してください。'));
            }
        });
    }
